...
ItemEvents.rightClicked((event) => {
	...
	for (let i = 0; i < playerName.length; i++) {
		...
		if (player.mainHandItem === item.id &&
			player.crouching &&
			player.mainHandItem !== "minecraft:air" &&			player.username === playerName[i]) {
...
PlayerEvents.chat((event) => {
	...
	for (let i = 0; i < playerName.length; i++) {
		// 输入-kl删除所有掉落物
		if (message.trim().equalsIgnoreCase('-kl') && player.username === playerName[i]) {
		...
		if (message.trim().equalsIgnoreCase('-kf') && player.username === playerName[i]) {
		...
		// 输入-efc清除自身所有buff
		if (message.trim().equalsIgnoreCase('-efc') && player.username === playerName[i]) {
		...
		// 输入-kla清除玩家之外的所有实体
		if (message.trim().equalsIgnoreCase('-kla') && player.username === playerName[i]) {
