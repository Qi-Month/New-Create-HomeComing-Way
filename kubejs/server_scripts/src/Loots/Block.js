LootJS.modifiers((loots) => {
	...
	loots.addBlockLootModifier('minecraft:grass')
		.addAlternativesLoot(
			LootEntry.of('new_create:grass_fiber').when((loots) =>
				loots.randomChance(0.6)
			)
		)
	...
	loots.addBlockLootModifier('#forge:sand')
		.addAlternativesLoot(
			LootEntry.of('new_create:pure_quartz').when((loots) =>
				loots.randomChance(0.01)
			)
		)
	...
	loots.addBlockLootModifier('minecraft:campfire')
		.removeLoot(all)
		.addAlternativesLoot(
			LootEntry.of('minecraft:campfire').when((loots) =>
				loots.randomChance(0.5)
			),
	...
	loots.addBlockLootModifier('minecraft:soul_campfire')
		.removeLoot(all)
		.addAlternativesLoot(
			LootEntry.of('minecraft:soul_campfire').when((loots) =>
				loots.randomChance(0.5)
			),
			LootEntry.of('minecraft:soul_soil').when((loots) =>
				loots.randomChance(1)
			),
			LootEntry.of('minecraft:soul_sand').when((loots) => loots.randomChance(1))
		)

	// 树叶
	loots.addBlockLootModifier('#minecraft:leavesf').addAlternativesLoot(
		LootEntry.of('minecraft:stick').when((loots) => loots.randomChance(0.3)),
		LootEntry.of('minecraft:apple').when((loots) => loots.randomChance(0.2))
	)
